...
export default defineNuxtConfig({
  ...
  nitro: {
    preset: 'node-server', // или 'node', вместо 'nitro-prerender'
    sourceMap: false,
    // prerender: {
    //   routes: [
    //     '/',
    ...
    //     '/reset-password',
    //   ],
    //   crawlLinks: true,  // ← ИЗМЕНЕНО с true на false
  ...
  routeRules: {
    ...
    "/policy": { ssr: true },
    "/offer": { ssr: true },
    "/verify-email": { ssr: false }, // Только клиент
    "/login": { ssr: true },
    "/register": { ssr: true },
    "/terms": { ssr: true },
    "/privacy": { ssr: true },
    "/recipes/**": { ssr: true }, // Динамические страницы
    "/blog/**": { ssr: true },
    "/cabinet/**": { ssr: false }, // Личный кабинет только на клиенте
    "/admin/**": { ssr: false }
  },
