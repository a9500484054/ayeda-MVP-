...
const seoPreview = computed(() => ({
  title: formData.value.seo.title || formData.value.title || 'Заголовок страницы',
  url: `https://xn--80aaaqndflmgf7a.xn--p1ai/recipe/${formData.value.srcPath || '...'}`,
  description: formData.value.seo.description || formData.value.description?.slice(0, 160) || 'Описание страницы'
}))
...
const transNameSrc = () => {
  if (formData.value.title) {
    ...
    // Автоматически заполняем SEO заголовок, если он пустой
            ...
            <UFormField label="Фото">
              <div class="space-y-3">
                ...
                <div v-if="formData.photo.src" class="mt-2">
                  <img :src="`${API_BASE_URL}${formData.photo.src}`" alt="Фото рецепта" class="max-w-[200px] h-auto rounded" />
                </div>
              </div>
            </UFormField>
              ...
              <div class="space-y-4">
                ...
                >
                  ...
                  <div v-if="step.image" class="flex items-start gap-3">
                    <img :src="`${API_BASE_URL}${step.image}`" alt="Фото шага" class="w-32 h-32 object-cover rounded" />
                    <UButton
                      color="error"
                      variant="outline"
                      size="sm"
              ...
              <div class="space-y-4 p-4 border rounded-lg">
                ...
                <div class="space-y-2">
                  ...
                  <USelect
                    ...
                    placeholder="Введите ключевые слова и нажмите Enter"
                    multiple
                    creatable
                    searchable
                    :items="formData.seo.keywords.map(k => ({ label: k, value: k }))"
                  />
                  <div class="text-xs text-muted-foreground">
                    Введите ключевое слово и нажмите Enter для добавления
                  </div>
                </div>
      ...
      <template #content>
        <div class="p-6">
          ...
          <div class="space-y-4">
            ...
            <!-- SEO информация в просмотре -->
            <div v-if="selectedRecipe?.seo?.title || selectedRecipe?.seo?.description" class="border-t pt-4">
              <h3 class="font-semibold mb-2">SEO информация</h3>
              <div class="space-y-1 text-sm">
                <div v-if="selectedRecipe.seo.title">
                  <span class="text-muted-foreground">Заголовок:</span>
                  <span class="ml-2">{{ selectedRecipe.seo.title }}</span>
                </div>
                <div v-if="selectedRecipe.seo.description">
                  <span class="text-muted-foreground">Описание:</span>
                  <span class="ml-2">{{ selectedRecipe.seo.description }}</span>
                </div>
                <div v-if="selectedRecipe.seo.keywords?.length">
                  <span class="text-muted-foreground">Ключевые слова:</span>
                  <span class="ml-2">{{ selectedRecipe.seo.keywords.join(', ') }}</span>
                </div>
              </div>
            </div>
          </div>
